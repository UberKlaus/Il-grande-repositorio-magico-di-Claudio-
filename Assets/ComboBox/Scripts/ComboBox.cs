	...
	public class ComboBox : MonoBehaviour 
	{
		...
		private void ToggleComboBox(bool directClick)
		{
			...
			if (overlayGO.activeSelf)
			{
				...
				var comboYPosition = rectTransform.position.y;
				var yPos = comboYPosition < buttonHeight * (_itemsToDisplay + 0.5f) ? comboYPosition + buttonHeight * 0.5f : comboYPosition - buttonHeight * (_itemsToDisplay + 0.5f);
				scrollPanelRectTransfrom.position = new Vector3(scrollPanelRectTransfrom.position.x, yPos, scrollPanelRectTransfrom.position.z);
				FixScrollOffset();
			}
			else
			{
